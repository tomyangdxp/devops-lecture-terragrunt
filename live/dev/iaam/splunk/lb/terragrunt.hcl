...
terraform {
  source = "../../../../../modules/iaam/azurerm_lb"
}
...
# Load Balancer must be created inside a resource group
...
# Set the input variables for the Load Balancer module
inputs = {
  location            = local.location
  resource_group_name = dependency.resource_group.outputs.resource_name
  environment = local.env
  subnets = dependency.subnet.outputs.subnets
  public_ip_address_dsOutboundlb_id = dependency.public_ip.outputs.dsOutboundlb_id
}
